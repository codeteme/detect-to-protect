"""
Two-stream VideoMAE RGB + Segmentation late fusion inference script.

Usage:
    python src/predict_videomae_seg.py
    python src/predict_videomae_seg.py \
        --checkpoint-path outputs/best_videomae_seg_ofs0p0.pt \
        --submission-path outputs/submission_videomae-seg-ofs0.0.csv
"""
...
CKPT_PATH = OUT_DIR / "best_videomae_seg_ofs0p0.pt"
SUBMISSION_PATH = OUT_DIR / "submission_videomae-seg.csv"
...
class TwoStreamTestDataset(Dataset):
    def __init__(self, csv_path, frames_dir, seg_dir, processor, clip_len):
    ...
    def __getitem__(self, idx):
        ...
        seg_rgb = np.stack([seg_clip] * 3, axis=-1)
        ...
        return rgb_pixels, seg_pixels, video_id
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run two-stream VideoMAE RGB+Seg prediction")
...
def main():
    ...
    test_ds = TwoStreamTestDataset(TEST_CSV, FRAMES_DIR, SEG_DIR, processor, clip_len)
    ...
    all_ids, all_probs = [], []
    with torch.no_grad():
        for rgb_pixels, seg_pixels, batch_ids in tqdm(test_loader, desc="Inference"):
            rgb_pixels = rgb_pixels.to(device, non_blocking=True)
            seg_pixels = seg_pixels.to(device, non_blocking=True)
            probs = torch.sigmoid(model(rgb_pixels, seg_pixels))
