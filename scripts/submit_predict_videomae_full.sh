#!/bin/bash
#SBATCH --job-name=dtp-videomae-full-predict
#SBATCH --partition=courses-gpu
#SBATCH --gres=gpu:1
#SBATCH --mem=32G
#SBATCH --cpus-per-task=4
#SBATCH --time=02:00:00
#SBATCH --output=logs/predict_videomae_full_%j.out
#SBATCH --error=logs/predict_videomae_full_%j.err
...
RUN_NAME=${RUN_NAME:-videomae-full-ofs${ANCHOR_OFFSET_SEC}}
OFFSET_TAG=${ANCHOR_OFFSET_SEC//./p}
OFFSET_TAG=${OFFSET_TAG//-/m}

CHECKPOINT_PATH=${CHECKPOINT_PATH:-outputs/best_videomae_full_ofs${OFFSET_TAG}.pt}
...
$PYTHON_BIN -u src/predict_videomae_full.py \
